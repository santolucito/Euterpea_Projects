...
defaultOutput does not work on mac, getDefaultOutputDeviceID (fromPOrtMidi) returns Just 4294967295
 
> setup :: IO ()
> setup = do
>   forkIO $ forever $ defaultOutput outputMidi' ()
>   return ()
> 
> outputMidi' id x = outputMidi id

The function we will use to do the actual live coding.

> playLive id m = do
>   mapM_ (defaultOutput deliverMidiEvent ) (toMidi' m)
