...
kinect_start dataChan = do
  ...
  glThread dataChan depthGrid triggered mode
...
glThread :: TVar Int -> IORef (Maybe (Vector Word16)) -> MVar () -> IORef Bool -> IO ()
glThread dataChan depthGrid triggered mode = do
  ...
  displayCallback $= display dataChan depthGrid triggered mode
  clearColor $= Color4 0.2 0.2 0.2 0
  mainLoop

n=2^11 - 1000

display ::  TVar Int -> IORef (Maybe (Vector Word16)) -> MVar () -> IORef Bool -> IO ()
display dataChan depthGrid triggered mode = do
